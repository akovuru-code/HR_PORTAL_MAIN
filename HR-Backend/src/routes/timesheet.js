...
router.post('/upload', upload.single('timesheet'), (req, res, next) => {
  ...
  next();
}, authenticateToken, timesheetController.uploadTimesheet);

router.get('/entries', authenticateToken, timesheetEntryController.getEntries);
router.post('/entries/submit', authenticateToken, timesheetEntryController.submitEntries);
