...
import EmpSidebar from "./Emp_sidebar";
import { FaUser, FaHome, FaCog, FaSignOutAlt, FaChevronDown, FaBuilding } from "react-icons/fa";
...
// List of pages for search navigation
const pages = [
...
export default function EmpLayout({ children }) {
    ...
    const goHome = () => navigate("/employee/dashboard");
    const goProfile = () => navigate("/employee/profile");
    const goSettings = () => navigate("/employee/settings");
                ...
                <div className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-200 relative flex-shrink-0">
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            onClick={() => navigate(-1)}
                        >
                            <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-600"><path d="M15 18l-6-6 6-6" /></svg>
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="relative" ref={profileRef}>
                            ...
                            >
                                ...
                                <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`ml-1 text-gray-400 transition-transform duration-200 ${profileOpen ? 'rotate-180' : ''}`}><path d="M6 9l6-6M6 3v6h6" /></svg>
                            </div>
                            {/* Dropdown - Updated UI */}
                            <div className={`absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border z-50 transition-all duration-200 ${profileOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-2 pointer-events-none'}`} style={{ minWidth: 320 }}>
                                ...
                                {/* Avatar overlaps header and white */}
                                ...
                                <div className="flex flex-col gap-1 px-4 pb-4">
                                    <button onClick={goHome} className="flex items-center gap-3 w-full px-4 py-2 rounded-lg text-gray-800 hover:bg-blue-100 transition text-base">
                                        <FaHome className="text-lg" /> Home
                                    </button>
                                    <button onClick={goProfile} className="flex items-center gap-3 w-full px-4 py-2 rounded-lg text-gray-800 hover:bg-blue-100 transition text-base">
                                        <FaUser className="text-lg" /> Profile
                                    </button>
                                    <button onClick={goSettings} className="flex items-center gap-3 w-full px-4 py-2 rounded-lg text-gray-800 hover:bg-blue-100 transition text-base">
                                        <FaCog className="text-lg" /> Settings
                                    </button>
                                    <button onClick={goCompany} className="flex items-center gap-3 w-full px-4 py-2 rounded-lg text-gray-800 hover:bg-blue-100 transition text-base">
                                        <FaBuilding className="text-lg" /> Company
                                    </button>
                                </div>
                                <div className="border-t px-4 py-3">
                                    <button
                                        className="flex items-center gap-3 text-red-600 hover:text-red-800 font-semibold py-2 w-full justify-center"
