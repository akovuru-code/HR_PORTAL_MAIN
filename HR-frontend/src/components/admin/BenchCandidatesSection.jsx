...
export default function BenchCandidatesSection() {
  ...
  const uploadResume = async (candidate, file) => {
    ...
    try {
      ...
      await request(`/api/company/bench-candidates/${candidate.id}/resume`, { method: 'POST', headers: headers(false), body: form }); await loadCandidates();
  ...
  const previewResume = async candidate => {
    try {
      ...
      const objectUrl = URL.createObjectURL(await response.blob()); window.open(objectUrl, '_blank', 'noopener,noreferrer'); window.setTimeout(() => URL.revokeObjectURL(objectUrl), 60000);
    } catch (err) { setError(err.message); }
  };
