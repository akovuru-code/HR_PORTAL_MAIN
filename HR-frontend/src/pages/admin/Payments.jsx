...
function ReceivePaymentModal({ open, onClose, onSaved, vendors, referenceNumberRequired }) {
  ...
  const submit = async event => {
    ...
    if (referenceNumberRequired && !form.referenceNumber.trim()) next.referenceNumber = "Reference Number is required.";
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onMouseDown={event => { if (dialogRef.current && !dialogRef.current.contains(event.target)) onClose(); }}>
    <div ref={dialogRef} role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl sm:p-8">
      ...
      <form onSubmit={submit} noValidate><div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <label>Reference Number {referenceNumberRequired ? <span className="text-red-500">*</span> : <span className="text-gray-500">(Optional)</span>}<input name="referenceNumber" value={form.referenceNumber} onChange={setField} className={inputClass("referenceNumber")} />{errors.referenceNumber && <p className="text-xs text-red-600">{errors.referenceNumber}</p>}</label>
      </div>
...
export default function AdminPayments() {
  ...
  const isRootAdmin = user?.accountType === 'root_admin';
  const isAccountsAdmin = String(user?.adminRole || user?.admin_role || '').toLowerCase() === 'accounts';
  const referenceNumberRequired = !isRootAdmin && !isAccountsAdmin;
