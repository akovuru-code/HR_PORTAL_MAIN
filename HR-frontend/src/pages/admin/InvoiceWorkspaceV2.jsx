...
export default function InvoiceWorkspaceV2({ employeeId: scopedEmployeeId }) {
  ...
  return <div className="max-w-7xl mx-auto px-4 py-8">
    ...
    <div className="overflow-x-auto bg-white border rounded-xl shadow-sm"><table className="min-w-full text-sm"><thead className="bg-gray-50"><tr>{['Invoice Number', ...(!isScoped ? ['Employee'] : []), 'Bill To', 'Invoice Date', 'Hours', 'Rate', 'Amount', 'Due Date', 'Status', 'Actions'].map(title => <th key={title} className="p-3 text-left">{title}</th>)}</tr></thead><tbody>{filtered.map(invoice => <tr className="border-t" key={invoice.id}><td className="p-3">{invoice.invoiceNumber}</td>{!isScoped && <td className="p-3">{invoice.employeeName}</td>}<td className="p-3">{invoice.billToCompany}</td><td className="p-3">{invoice.invoiceDate}</td><td className="p-3">{invoice.items?.[0]?.hours}</td><td className="p-3">{invoice.items?.[0]?.rate}</td><td className="p-3">{invoice.total}</td><td className="p-3">{invoice.dueDate}</td><td className="p-3">{invoice.status}</td><td className="p-3 whitespace-nowrap space-x-2"><button className="text-blue-700" onClick={() => edit(invoice)}>Edit</button>{invoice.pdfUrl && <><button className="text-blue-700" onClick={() => openPdf(invoice.pdfUrl)}>Preview</button><button className="text-blue-700" onClick={() => downloadPdf(invoice)}>Download</button></>}<button className="text-blue-700" onClick={() => regenerate(invoice)}>Regenerate</button></td></tr>)}{!filtered.length && <tr><td colSpan={isScoped ? '9' : '10'} className="p-8 text-center text-gray-500">No invoices found.</td></tr>}</tbody></table></div>
