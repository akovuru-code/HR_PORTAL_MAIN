...
export default function InvoiceWorkspaceV2({ employeeId: scopedEmployeeId }) {
  ...
  const filtered = useMemo(() => invoices.filter(invoice => (!filters.number || invoice.invoiceNumber?.toLowerCase().includes(filters.number.toLowerCase())) && (isScoped || !filters.employee || invoice.employeeName?.toLowerCase().includes(filters.employee.toLowerCase())) && (!filters.billTo || invoice.billToCompany?.toLowerCase().includes(filters.billTo.toLowerCase())) && (!filters.status || (filters.status === 'Overdue' ? invoice.isOverdue : filters.status === 'Due' ? invoice.isDue : invoice.status === filters.status)) && (!filters.currency || invoice.currency === filters.currency) && (!filters.from || invoice.invoiceDate >= filters.from) && (!filters.to || invoice.invoiceDate <= filters.to)), [invoices, filters, isScoped]);
