...
const WorkClient = forwardRef(function WorkClient({ goBack, parentClient, parentVendor, parentPrime }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    ...
    saveDraft: async () => {
      ...
      } catch (err) { console.error('[WorkClient] ref.saveDraft error:', err?.response?.data || err.message); }
    },
  }));
  ...
  const handleBack = async () => {
    ...
    } catch (err) { console.error('[WorkClient] handleBack save error:', err?.response?.data || err.message); }
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const serverClients = onboardingRes?.data?.workClientDetails;
        if (serverClients && serverClients.length > 0) {
          const clients = serverClients.filter(c => c.type === 'client').map(c => ({
            ...initialClient, name: c.name || '', address: c.address || '',
            startDate: c.start_date || '', endDate: c.end_date || '',
            workEmail: c.work_email || '', managerEmail: c.manager_email || '',
            managerPhone: c.manager_phone || '', remoteWorkLocation: c.remote_work_location || '',
            docFile: c.doc_file || null,
          }));
          const vendors = serverClients.filter(c => c.type === 'vendor').map(v => ({
            ...initialVendor, name: v.name || '', address: v.address || '',
            startDate: v.start_date || '', endDate: v.end_date || '',
            parentName: v.contact_person || '', email: v.email || '',
            phone: v.phone || '', finc: v.fein || '', docFile: v.doc_file || null,
          }));
          const primes = serverClients.filter(c => c.type === 'primeVendor').map(p => ({
            ...initialPrime, name: p.name || '', address: p.address || '',
            startDate: p.start_date || '', endDate: p.end_date || '',
            email: p.email || '', phone: p.phone || '', docFile: p.doc_file || null,
          }));
      ...
      try {
        const draft = await getDraft(employeeId, 'workClient');
        if (draft?.data?.payload) {
          const payload = draft.data.payload;
          if (payload.clientInfo) { setClientInfo(payload.clientInfo); loaded = true; }
          if (payload.vendorInfo) { setVendorInfo(payload.vendorInfo); loaded = true; }
          if (payload.primeInfo) { setPrimeInfo(payload.primeInfo); loaded = true; }
      ...
      if (!loaded && (parentClient || parentVendor || parentPrime)) {
        if (parentClient?.name) setClientInfo([{ ...initialClient, name: parentClient.name, startDate: parentClient.startDate || '', endDate: parentClient.endDate || '' }]);
        if (parentVendor?.name) setVendorInfo([{ ...initialVendor, name: parentVendor.name, startDate: parentVendor.startDate || '', endDate: parentVendor.endDate || '' }]);
        if (parentPrime?.name) setPrimeInfo([{ ...initialPrime, name: parentPrime.name, startDate: parentPrime.startDate || '', endDate: parentPrime.endDate || '' }]);
      }
    }
    loadData();
  }, [user]);
        ...
        {clientInfo.map((c, idx) => (
          <div key={idx} className={`mb-4 pb-4 ${idx !== clientInfo.length - 1 ? 'border-b' : ''}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
              ...
              <input type="text" placeholder="Client Manager Phone" value={c.managerPhone} onChange={e => setClientInfo(info => info.map((ci, i) => i === idx ? { ...ci, managerPhone: e.target.value } : ci))} className="border rounded px-3 py-2 text-sm w-full max-w-md" disabled={isReadOnly} />
        ...
        {vendorInfo.map((v, idx) => (
          <div key={idx} className={`mb-4 pb-4 ${idx !== vendorInfo.length - 1 ? 'border-b' : ''}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
              ...
              <input type="text" placeholder="Vendor Phone" value={v.phone} onChange={e => setVendorInfo(info => info.map((vi, i) => i === idx ? { ...vi, phone: e.target.value } : vi))} className="border rounded px-3 py-2 text-sm w-full max-w-md" disabled={isReadOnly} />
        ...
        {primeInfo.map((p, idx) => (
          <div key={idx} className={`mb-4 pb-4 ${idx !== primeInfo.length - 1 ? 'border-b' : ''}`}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2">
              ...
              <input type="tel" placeholder="Prime Vendor Phone" value={p.phone} onChange={e => setPrimeInfo(info => info.map((pi, i) => i === idx ? { ...pi, phone: e.target.value } : pi))} className="border rounded px-3 py-2 text-sm w-full max-w-md" disabled={isReadOnly} />
            </div>
